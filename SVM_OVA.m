...
for k=1:4
    ...
    svmModel= svmtrain(train,trainlable,'kernel_function','rbf');
    eval(['model',num2str(k),'=','svmModel']);
end

for k=1:4
    eval(['svmModel','=','model',num2str(k),]);
    predictlable(:,k)= svmclassify(svmModel,test);

end
 [~,predictlable] = max(predictlable,[],2);%cluster label
count=0;
for i=1:size(Y2,1)
